import type { Assignment, Difficulty, GameState, WordBank, WordMode, WordPair } from '../types';
...
export function maxImpostors(playerCount: number) {
  return Math.max(1, Math.floor((playerCount - 1) / 2));
}

export function recommendedImpostors(playerCount: number) {
  if (playerCount >= 10) return 3;
  if (playerCount >= 7) return 2;
  return 1;
}

export function pairKey(category: string, pair: WordPair) {
  return `${category}:${pair.w.join('|')}`;
}

export function pairMatchesDifficulty(pair: WordPair, difficulty: Difficulty) {
  if (difficulty === 'misto') return true;
  if (difficulty === 'facil') return pair.s <= 3 || pair.d === 'f';
  return pair.s >= 4 || pair.d === 'd';
}

export function getPairsForDifficulty(bank: WordBank, category: string | null, difficulty: Difficulty) {
  if (!category || !bank[category]) return [];
  return bank[category].filter((pair) => pairMatchesDifficulty(pair, difficulty));
}

export function getSetupStats(bank: WordBank, category: string | null, difficulty: Difficulty, recentPairKeys: string[] = []) {
  const availablePairs = getPairsForDifficulty(bank, category, difficulty);
  const recentSet = new Set(recentPairKeys);
  const freshPairs = availablePairs.filter((pair) => category && !recentSet.has(pairKey(category, pair)));
...
export function createRound(state: GameState, bank: WordBank): GameState {
  if (state.players.length < 3 || !state.category || !bank[state.category]?.length) return state;

  const category = state.category;
  const allPairs = bank[category];
  const filteredPairs = getPairsForDifficulty(bank, category, state.difficulty);
  ...
  const recentSet = new Set(recentPairKeys);
  const freshPairs = usablePairs.filter((pair) => !recentSet.has(pairKey(category, pair)));
  ...
  } while (candidatePairs.length > 1 && pairKey(category, selectedPair) === state.lastPairKey);

  const selectedPairKey = pairKey(category, selectedPair);
  ...
  const impostorWord = getImpostorWord(state.wordMode, category, similarWord);
  const shuffledPlayers = [...state.players].sort(() => Math.random() - 0.5);
  const impostorSet = new Set<number>();
...
function getImpostorWord(wordMode: WordMode, category: string, similarWord: string) {
...
export type BoardEdge = [number, number];

export function buildBoard(assignments: Assignment[], caseId: string) {
